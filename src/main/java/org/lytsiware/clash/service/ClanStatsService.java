...
public class ClanStatsService implements IClanStatsService {
    ...
    public List<Player> calculateAvgs() {
        ...
        Map<Player, List<PlayerWeeklyStats>> allPlayerStats = playerWeeklyStatsRepository.findByWeek(startingWeek, latestWeek);

        //finds the players that are currently in the clan
        Set<Player> currentPlayers = playerWeeklyStatsRepository.findByWeek(latestWeek).stream().map(PlayerWeeklyStats::getPlayer).collect(Collectors.toSet());
        ...
        for (Player player : playerStats.keySet()) {
            double avgCardDonation = playerStats.get(player).stream().
                    collect(Collectors.summingInt(PlayerWeeklyStats::getCardDonation)) / (double) playerStats.get(player).size();
            double avgChestContribution = playerStats.get(player).stream().
                    collect(Collectors.summingInt(PlayerWeeklyStats::getChestContribution)) / (double) playerStats.get(player).size();
    ...
    public void updateDatabaseWithLatest() {
        // get the stats
        // convert to WeekStats
        // List<PlayerWeeklyStats> stats = new ArrayList<>();
        // playerWeeklyStatsRepository.saveOrUpdateAll(stats);
    ...
    public void initDb() {
        ...
        for (int pl = 0; pl < 10; pl++) {
            ...
            for (int w = 1; w < 11; w++) {
                ...
                stats.setWeek(DateWeekConverter.toWeek(LocalDate.now().minusWeeks(w)));
